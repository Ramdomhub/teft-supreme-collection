...
export default function PulsePage() {
  ...
  return (
    <main className="min-h-screen bg-[#0f1112] text-white p-8">
      <table className="w-full border-collapse">
        <thead>
          <tr className="text-left text-zinc-500 uppercase text-[10px] tracking-widest border-b border-white/5">
            <th className="px-8 py-4">Asset / Socials</th>
            <th className="px-8 py-4">Age</th>
            <th className="px-8 py-4">MCap</th>
            <th className="px-8 py-4 text-right">TEFT Score</th>
          </tr>
        </thead>
        <tbody>
          {signals.map((t, i) => (
            <tr key={i} className="border-b border-white/5">
              {/* DEIN SNIPPET 1: ASSET & SOCIALS */}
                ...
                <div className="flex items-center gap-4">
                  <div className="relative">
                    {t.image ? (
                      ...
                      <div className="w-12 h-12 rounded-full bg-zinc-800 flex items-center justify-center text-white font-black text-xs">{t.ticker[0]}</div>
                    )}
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      ...
                      <div className="flex gap-2 ml-2">
                        {t.twitter && (
                          <a href={t.twitter} target="_blank" className="p-1 bg-white/5 rounded-md hover:bg-white/10 text-zinc-400 hover:text-white transition-all">
                        ...
                        {t.telegram && (
                          <a href={t.telegram} target="_blank" className="p-1 bg-white/5 rounded-md hover:bg-white/10 text-zinc-400 hover:text-white transition-all">
              ...
              {/* DEIN SNIPPET 2: SCORE & BUY BUTTON */}
              <td className="px-8 py-6 text-right">
                <div className="flex flex-col items-end gap-2">
                  ...
                  <button className="bg-orange-500 hover:bg-orange-400 text-black px-5 py-2 rounded-xl text-[10px] font-black uppercase italic tracking-widest transition-all">
                     Buy {tradeSize}
                  </button>
                </div>
              </td>
