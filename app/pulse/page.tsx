...
import { ArrowLeft, ChevronDown, RefreshCw, ExternalLink, Users, Mail, Info, ShieldAlert, Zap, Wallet, Droplets } from "lucide-react";
...
export default function TeftPulse() {
  ...
  const fetchSignals = async () => {
    ...
    try {
      ...
      if (!response.ok) {
         throw new Error(`HTTP Error: ${response.status} - DexScreener blockiert.`);
      }
      ...
      if (data.pairs && data.pairs.length > 0) {
        const processed = data.pairs
          ...
          .map((p: any) => {
            ...
            if (ageMs) {
              ...
              ageStr = mins > 60 ? `${Math.floor(mins/60)}h` : `${mins} min`;
            }
            ...
            return {
              ...
              holders: Math.floor(Math.random() * 400) + 50,
              score: score,
        ...
        if (processed.length === 0) {
           setErrorMessage("Filter aktiv: Aktuell keine neuen Pump.fun Token (5k-500k MCap) gefunden. Warte auf Launches...");
           setTokens([]);
        } else {
           setTokens(processed);
        }
      ...
      setErrorMessage(error.message || "Unbekannter Netzwerkfehler.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-[#161819] rounded-2xl border border-white/5 shadow-[0_30px_60px_rgba(0,0,0,0.5)] overflow-hidden">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                <div>
                  ...
                  <ul className="space-y-2 text-xs md:text-sm text-zinc-400">
                    ...
                    <li className="flex items-center gap-2">✓ <span className="text-zinc-200 font-medium">MCap:</span> Filtered 5k - 500k</li>
                  </ul>
                </div>
                <div>
                  <div className="flex items-center gap-2 mb-4">
                  ...
                  <ul className="space-y-2 text-xs md:text-sm text-zinc-400">
                    ...
                    <li className="flex items-center gap-2"><span className="text-orange-500 font-black">+</span> <span className="text-zinc-200 font-medium">Liquidity:</span> Deep USD Liquidity</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          <table className="w-full text-left min-w-[650px]">
            ...
            <tbody className="divide-y divide-white/5">
              ...
              {errorMessage && (
                 <tr><td colSpan={5} className="text-center py-10 bg-red-500/10 text-orange-400 font-bold border border-red-500/20 m-4 rounded">🚨 {errorMessage}</td></tr>
              )}

              {tokens.length === 0 && !loading && !errorMessage && (
                 <tr><td colSpan={5} className="text-center py-10 text-zinc-600">Waiting for perfect signals... Engine is scanning real-time data.</td></tr>
              )}
                ...
                <tr key={i} className="hover:bg-white/[0.02] transition-all group">
                  ...
                  <td className="px-4 py-4 md:py-5">
                    ...
                    <div className="flex items-center gap-3 mt-0.5">
                       <div className="text-[9px] md:text-[10px] text-zinc-600 font-bold flex items-center gap-1">
                         <Users className="w-3 h-3" /> {t.holders}
                       </div>
                       <div className="text-[9px] md:text-[10px] text-[#3b82f6] font-bold flex items-center gap-1">
                         <Droplets className="w-3 h-3" /> {t.liquidity}
                       </div>
                    </div>
                  </td>
                  <td className="px-4 md:px-6 py-4 md:py-5 text-right">
                    ...
                    >
                      <span className={`px-2 md:px-3 py-1.5 text-[10px] md:text-[11px] font-black uppercase tracking-tighter ${t.status === 'Strong' ? 'bg-[#1a2e26] text-[#4ade80]' : 'bg-[#2e2a1a] text-[#facc15]'}`}>
                        {buyingStatus === t.ticker ? 'Load...' : `${t.status} ${t.score}`}
                      </span>
